...
NoCat::BrowserDetect - Determine Web browser, version, and platform from an HTTP user agent string
...
=head1 SYNOPSIS
    ...
    my $browser = NoCat::BrowserDetect->new($user_agent_string);
...
The NoCat::BrowserDetect object does a number of tests on an HTTP user agent
...
=head2 new()
    ...
    NoCat::BrowserDetect->new( $user_agent_string )
...
call NoCat::BrowserDetect::method_name(). You will then be working with a
default NoCat::BrowserDetect object that is created behind the scenes.
...
creating a new NoCat::BrowserDetect object each time.
